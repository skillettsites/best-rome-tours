...
import { getMonthBySlug, monthStrip } from '@/data/rome-months';
import { romeClimate } from '@/data/rome-climate';
...
export function buildMonthPage(slug: string) {
  ...
  const climate = romeClimate[data.monthKey];
  ...
  const articleSchema = {
    ...
    '@type': 'Article',
    headline: `Rome in ${data.month}: Weather, Events and the Best Tours`,
  ...
  const itemListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `The best tours in Rome in ${data.month}`,
    ...
    <>
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8">
        <Breadcrumbs
          items={[
            { label: 'Home', href: '/' },
            { label: 'Best time to visit Rome', href: HUB_PATH },
            { label: `Rome in ${data.month}` },
          ]}
        />
        ...
        <article>
          <header>
            <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 leading-tight">
              Rome in {data.month}: Weather, Events and the Best Tours
            </h1>
          ...
          <section aria-labelledby="quick-pick" className="mt-8">
            <h2 id="quick-pick" className="text-xl font-bold text-gray-900">
              Quick pick: the best Rome tours in {data.month}
            </h2>
          ...
          {/* 1. Verdict */}
          <H2 id="is-it-good">Is {data.month} a good time to visit Rome?</H2>
          ...
          {/* 2. Weather */}
          <H2 id="weather">Rome weather in {data.month}</H2>
          <Capsule>{data.weather.capsule}</Capsule>
          <div className="mt-4 overflow-x-auto rounded-lg border border-gray-200">
            <table className="w-full min-w-[560px] text-left text-sm">
              <caption className="sr-only">Rome climate averages for {data.month}</caption>
              <thead className="bg-gray-50 text-gray-700">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100">
                <tr>
                  ...
                  <td className="px-3 py-2.5 text-gray-600">Ranges from 11.9C in January to 31.4C in August</td>
                </tr>
                <tr>
                  ...
                  <td className="px-3 py-2.5 text-gray-600">Ranges from 3.5C in January to 19.8C in August</td>
                </tr>
                <tr>
                  <th scope="row" className="px-3 py-2.5 font-medium text-gray-900">Rain days</th>
                  <td className="px-3 py-2.5 text-gray-800">{climate.rainDays} days, {climate.rainMm}mm</td>
                  <td className="px-3 py-2.5 text-gray-600">Driest is July at 2 days, wettest is November at 9</td>
                </tr>
                <tr>
                  ...
                  <td className="px-3 py-2.5 text-gray-600">From 9h 09m in December to 15h 12m in June</td>
                </tr>
                <tr>
                  <th scope="row" className="px-3 py-2.5 font-medium text-gray-900">Sunshine per day</th>
                  <td className="px-3 py-2.5 text-gray-800">{climate.sunHoursPerDay} hours</td>
                  <td className="px-3 py-2.5 text-gray-600">From 4 hours in December to 11 hours in July</td>
                </tr>
                <tr>
                  <th scope="row" className="px-3 py-2.5 font-medium text-gray-900">Sea temperature</th>
                  <td className="px-3 py-2.5 text-gray-800">{climate.seaTempC}C</td>
                  <td className="px-3 py-2.5 text-gray-600">Tyrrhenian coast at Rome, peaks at 26C in August</td>
                </tr>
              </tbody>
            </table>
          </div>
          <p className="mt-2 text-xs text-gray-500">
            Temperature, rainfall, rain days, sunshine and sea temperature:{' '}
          ...
          {/* 4. Tours */}
          <H2 id="best-tours">The best tours in Rome in {data.month}</H2>
          ...
          <div className="mt-4 overflow-x-auto rounded-lg border border-gray-200">
            <table className="w-full min-w-[640px] text-left text-sm">
              <caption className="sr-only">Dated events in Rome in {data.month} {year}</caption>
              <thead className="bg-gray-50 text-gray-700">
                <tr>
          ...
          <div className="mt-4 overflow-x-auto rounded-lg border border-gray-200">
            <table className="w-full min-w-[640px] text-left text-sm">
              <caption className="sr-only">Closures affecting Rome sights in {data.month}</caption>
              <thead className="bg-gray-50 text-gray-700">
                <tr>
          ...
          <p className="mt-2 text-xs text-gray-500">
            Vatican opening days, hours and closure dates:{' '}
            <a href={VATICAN_SOURCE.url} target="_blank" rel="noopener noreferrer nofollow" className="underline">
              {VATICAN_SOURCE.label}
            </a>
            . The only official Vatican Museums ticket seller is tickets.museivaticani.va.
          </p>
          ...
          {/* 7. Packing */}
          <H2 id="what-to-pack">What to wear and pack for Rome in {data.month}</H2>
          ...
          <Capsule>
            ...
            experiences that actually work in {data.month}&apos;s weather and daylight sit highest. Every price,
          ...
          <ol className="mt-4 space-y-3 text-gray-700 leading-relaxed list-decimal pl-5">
            ...
            <li>
              ...
              for a specific, stated reason tied to that month: heat, rain days, daylight hours, sunset time
              or a closure. Every move is explained in the entry itself rather than left implied.
            </li>
            <li>
              <span className="font-semibold text-gray-900">Nothing is invented.</span> Prices, star ratings
              and review counts come from the live product page. We do not publish an opening time, closure or
              festival date we could not confirm from an official source, and we say so where we could not.
            </li>
            <li>
          ...
          {/* 9. FAQs */}
          <H2 id="faqs">FAQs about Rome in {data.month}</H2>
          ...
          <nav aria-label="Other months" className="mt-12">
            <div className="flex flex-col sm:flex-row gap-3 justify-between">
              ...
              >
                ...
                <span className="block font-semibold text-blue-900">Rome in {data.prevMonth}</span>
              </Link>
              <Link
                href={data.nextPath}
              ...
              >
                ...
                <span className="block font-semibold text-blue-900">Rome in {data.nextMonth}</span>
              </Link>
            </div>

            <div className="mt-6 rounded-lg bg-gray-50 border border-gray-200 p-4">
              <p className="text-sm font-semibold text-gray-900">Rome month by month</p>
...
export function buildMonthMetadata(slug: string) {
  ...
  if (!data) return {};
  const year = seasonYear(data.yearOffset);
  const title = `Rome in ${data.month} ${year}: Weather, Events and the Best Tours`;
  return {
